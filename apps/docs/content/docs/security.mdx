...
<Callout type="warn">
  `onOrgInvitation` is the one exception to tier two. Organization invitations have no
  bundled template, so **no email is sent at all** unless you provide the hook. Enabling
  [organizations](/docs/organizations) without wiring `onOrgInvitation` produces invitations
  that exist in the database and never reach an inbox.
</Callout>
...
| `onOrgInvitation` | `{ email, invitationId, orgName, orgSlug, role, acceptUrl, token }` | An [organization](/docs/organizations) invitation was created, from the account console or the Admin API. **No bundled fallback** — without this hook nothing is sent. |
