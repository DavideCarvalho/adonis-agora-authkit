...
Five buckets exist, each with its own budget, and they are separate on purpose — two
different questions must not consume one another's allowance:
...
| `otpLogin` | IP | 5 / min | `POST /auth/interaction/:uid/otp-verify`. Tighter than `login` because a [six-digit code](/docs/passwordless#email-login-codes-otp) is guessable in a way a token is not. |
| `sudo` | IP | 10 / min | The [sudo-mode](#sudo-mode) confirmation routes. Same numbers as `login`, separate budget: `login` measures an anonymous stranger guessing credentials, `sudo` measures a signed-in user re-proving themselves. |
...
The email address is the account identity, so **it has to be normalized the same way on
both sides** — where the account is written and where it is looked up. AuthKit applies one
conservative rule, `normalizeEmailIdentifier`: `trim()` + `toLowerCase()`, nothing else.

```ts
import { normalizeEmailIdentifier } from '@adonis-agora/authkit-server';

normalizeEmailIdentifier('  Davi.Carvalho96+lastro@Gmail.com  ');
// → 'davi.carvalho96+lastro@gmail.com'  (dots and +tag preserved)
```

The address someone typed is their identity. AuthKit does **not** decide that
`a.b@gmail.com` and `ab@gmail.com` are the same person, and it does not drop the
subaddress they chose to use.

<Callout type="warn">
  **Changed in 0.69.** Until 0.68 signup ran the address through VineJS'
  `.normalizeEmail()` (the validator.js defaults), which for Gmail **removed dots and the
  `+tag`**, while the login identifier step normalized nothing and matched exactly. Accounts
  were stored under an address different from the one typed, and their owners could not log
  in — with no error message, because the login screen is enumeration-proof. If you depended
  on the old collapsing behavior (two spellings sharing one account), that no longer happens:
  new signups keep the address as typed.
</Callout>

### Migrating the stored addresses — a required upgrade step

Changing the rule does not by itself rescue the accounts that were **already stored** under
another spelling: the ones the old signup mangled, and the ones import, invitations or a
social provider wrote with mixed case. The login looks up **one** form — the normalized one
— so those accounts are unreachable, and, because the screen is enumeration-proof, their
owners get no error message at all. There is no fallback in the login path on purpose:
every extra spelling tried is an extra query per unknown address, which is exactly the
attack path.

<Callout type="warn">
  **Required on upgrade from 0.70 or earlier.** Run `authkit:users:normalize-emails` when
  you upgrade. Skip it and any account stored under a mangled or mixed-case address stays
  locked out, silently. Version 0.70 shipped a `login.legacyEmailFallback` bridge that
  tried alternative spellings at login time; **that option is gone** — delete it from your
  config (an unknown key is ignored, but it no longer does anything).
</Callout>

```sh
# Report only — writes nothing. Shows what would change and, crucially, the collisions.
node ace authkit:users:normalize-emails

# Apply. Refuses to touch any account involved in a collision.
node ace authkit:users:normalize-emails --apply
```

A **collision** is two accounts that collapse onto the same normalized address — say
`Davi@Acme.com` and `davi@acme.com`. The migration never merges accounts and never picks a
winner: it leaves **both** untouched, lists them, and exits non-zero so an operator has to
decide. Everything else is rewritten in place; `email_verified_at` is left as it was,
because the mailbox is the same one and only the stored spelling changed.

Writing needs `AccountEmailRewriteCapability` on the account store (the bundled Lucid store
has it). A store without it still gets the report — `--apply` refuses and you normalize the
column yourself.

<Callout type="info">
  The same address has to be normalized on the **write** side too, or the migration is a
  one-off with a leak: signup, "forgot password", email change, admin user creation, org
  invitations, `authkit:users:import` and social signup all go through
  `normalizeEmailIdentifier` already. If you write accounts through your own code path, do
  the same there.
</Callout>
...
absence of a valid [trusted-device](/docs/mfa#trusted-devices) cookie — it works whether or
not the user ever ticked "trust this device".
...
- The sign-up page displays the "registration is currently disabled" message from the [i18n catalog](/docs/i18n) (`errors.registration_disabled`).
...
**Audit:** writing the setting — from the admin console or the Admin REST API — emits a
`settings.updated` [audit event](/docs/events#event-types) whose `metadata.key` is
`maintenance_mode`.
...
[Admin Console — Authentication methods card](/docs/admin-console#settings-page))
...
The link carries a single-use, high-entropy token; only a hash of it is ever persisted, and
...
default mailer, with branding and i18n applied — see [Mail hooks](#mail-hooks) for the
...
Every email AuthKit sends goes out through the same two-tier arrangement, and understanding
...
[branding](/docs/customizing-auth), and the [i18n catalog](/docs/i18n) already applied. Only
...
Every hook is best-effort and fire-and-forget: a throwing hook never breaks the flow that
triggered it.
...
| `onMagicLink` | `{ email, magicUrl, token, code?, channel? }` | A [magic link](/docs/passwordless#magic-link) was issued. `code` is present only when [email login codes](/docs/passwordless#email-login-codes-otp) are enabled; `channel` (`'code'` or `'link'`) carries the user's choose-first selection so you can render a code-only or link-only email. Both are absent in the plain magic-link flow. |
| `onSudoLink` | `{ email, sudoUrl }` | A [sudo-mode](#sudo-mode) confirmation link was requested. Distinct from `onMagicLink` on purpose: a magic link **authenticates**, a sudo link only grants sudo to someone already signed in, and the two tokens share nothing. |
| `onNewDeviceLogin` | `{ account, ip, userAgent, timestamp }` | A login succeeded from a device with no valid [trusted-device](/docs/mfa#trusted-devices) cookie. |
| `onOrgInvitation` | `{ email, invitationId, orgName, orgSlug, role, acceptUrl, token }` | An [organization](/docs/organizations) invitation was created, from the account console or the Admin API. Without the hook the bundled mailer sends the invitation, carrying the org name, the role being offered, and the accept link. |
...
If you have a single issuer — the ordinary case — you do not need this field, and setting it
to anything derived from an incoming request reopens exactly the hole the default closes.
...
### Resource indicators with opaque tokens

`resources` is also the **allowlist** of resource indicators (RFC 8707), and it works
with opaque tokens too — no JWT required. Clients that always send `resource` (MCP
clients do: the MCP authorization spec requires it) are accepted as long as the value is
declared:
...
accessTokens: {
  // format stays 'opaque'
  resources: {
    'https://app.acme.com/mcp': {}, // opaque, aud = the key
  },
},
```

- A declared `resource` (a trailing `/` is tolerated) is granted on consent and bound to
  the access token: the opaque token's `aud` is the resource, and it is still found by
  `provider.AccessToken.find` and introspectable.
- A `resource` that is not declared (nor the JWT `audience`) is rejected with
  `invalid_target` (RFC 8707 §2).
- Requests **without** `resource` are unchanged: in opaque mode they get the usual token,
  without `aud`, usable at `userinfo` — your web login does not move.
...
- **Account has MFA enrolled** → the second factor is required for that login. On success the
  id_token carries the real `acr` (`mfaAcr`) and `amr` (`[<primary>, 'mfa', <method>]`, e.g.
  `['pwd', 'mfa', 'totp']`; see [MFA — `amr`](/docs/mfa#what-the-id_token-says-amr)).
...
A step-up request **always ignores the [trusted-device](/docs/mfa#trusted-devices) cookie**.
...
[Trusted devices — step-up always ignores the cookie](/docs/mfa#step-up-always-ignores-the-cookie).
