...
Changing the rule does not by itself rescue the accounts that were already stored mangled.
When the normalized address finds nothing, the login identifier step (and the account
console login, and "forgot password") retries with the address exactly as typed and with
the legacy validator.js normalization — and accepts the result **only when those point at
exactly one account**. A tie is treated as "not found": the library does not guess.

It is on by default (`login.legacyEmailFallback: true`); turning it off locks those accounts
out again. It is a **temporary bridge** — migrate the stored addresses, then turn it off:

```ts
defineConfig({
  login: { legacyEmailFallback: false },
});
```

Nothing about it is observable from outside: same screen, same message, and the extra
lookups only happen when the compatibility spellings actually differ from the normalized
one. The same resolution guards the dedupe checks in signup, social login, admin user
creation and `authkit:users:import`, so none of them creates a *second* account for someone
who already has a mangled one.

<Callout type="warn">
  **What the bridge cannot reach.** It only tries spellings it can derive from what was
  typed. An account stored as `Davi@Acme.com` is reachable by someone typing
  `Davi@Acme.com`, but **not** by someone typing `davi@acme.com` — there all three forms
  collapse to the same string, and finding it would need a case-insensitive store query,
  i.e. a table scan on every login with an unknown address (exactly the attack path). Those
  accounts need their stored address migrated, not a bridge. If your table was populated by
  imports, invites or a provider that returns mixed case, normalize the column before
  relying on this.
</Callout>
