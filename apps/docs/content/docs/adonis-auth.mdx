...
<Cards>
  ...
  <Card title="My API is called with a bearer token" description="A native app, SPA or service sends Authorization: Bearer. Use oidcBearerGuard()." href="#an-api-called-with-a-bearer-access-token" />
</Cards>
...
## An API called with a bearer access token

Mobile apps (RFC 8252), SPAs talking straight to your API and services carry no web session — they
send `Authorization: Bearer <access_token>`. `oidcBearerGuard` verifies that token against the
AuthKit issuer (in-process, or as a remote resource server through JWKS / introspection) and
resolves the user with the same user provider:
...
`middleware.auth({ guards: ['web', 'api'] })` then accepts either. Failures are always a 401
(never a redirect), and `getAccountId(ctx)` falls back to the bearer identity when there is no
session. The full walkthrough — creating a native client, the Expo sign-in, refresh rotation,
options and verification modes — is in [Native apps](/docs/native-apps).
