...
All three resolvers (`jwt`, `pat`, `opaque`) populate `profile.avatarUrl`
(from `picture`) and `sessionId` (from `sid`), so these fields are available regardless of token
model. See [Resolvers](/docs/resolvers).

## resolveUser & resolveAppRoles

- **`resolveUser(identity, ctx)`** — map the identity to _your_ user record (create on
