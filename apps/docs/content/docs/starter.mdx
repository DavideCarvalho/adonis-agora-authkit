...
- **A primary key.** Neither `withAuthUser()` nor `withCredentials()` declares an `id`
  column or generates one — without a `@beforeCreate` hook assigning a real id, Lucid
  inserts `NULL` and the model's `id` comes back as the database's internal
  auto-increment rowid instead of a stable identifier, making the account unreachable by
  its real id on the very next request.
- **`fullName`.** The built-in signup screen collects a "Name" field, and the Lucid
  store passes it straight to `AuthUser.create()` — omit this column and the first
  signup throws `Cannot define "fullName" on "AuthUser" model, since it is not defined
  as a model property`.
...
You'll also need a migration for the `auth_users` table backing this model — it isn't
published by `node ace configure` (that only wires the config/provider/middleware, not
the schema for a model you own). Columns must match every `@column()` declared above
plus the two mixins' own (`email`, `password`, `globalRoles`); see
[Account Store](/docs/account-store) for the full column list.
...
<Callout type="warn">
  ...
  [Getting Started](/docs/getting-started#mount-the-host-manual) for the
...
```ts title="config/authkit.ts (IdP)"
import env from '#start/env'
import AuthUser from '#models/auth_user'
import {
  defineConfig,
...
<Callout type="warn">
  ...
  [Getting Started](/docs/getting-started#mount-the-host-manual).
</Callout>
