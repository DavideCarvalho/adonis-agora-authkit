...
- **A primary key.** Neither `withAuthUser()` nor `withCredentials()` declares an `id`
  column or generates one — without a `@beforeCreate` hook assigning a real id **and**
  `static selfAssignPrimaryKey = true`, Lucid inserts `NULL` for the `id` column, or
  (with only the hook) silently overwrites the id you just assigned with the
  database's internal auto-increment rowid right after the insert — either way the
  account is unreachable by its real id on the very next request.
...
[Account Store](/docs/account-store) for the full column list, and add your own
migration for any extra column your own `@column()` declarations introduce later.

```ts title="database/migrations/xxxx_create_auth_users_table.ts"
