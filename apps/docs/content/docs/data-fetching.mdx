...
| `admin.users.list(params?)` | `GET /users` | `{ search?, page?, size? }` |
...
| `admin.audit.list(params?)` | `GET /audit` | `{ type?, page?, size?, subject? }` |
...
| `useUsersQueryOptions(params?)` | `authkitKeys.admin.users(params)` | `{ search?, page?, size? }` |
...
| `useAuditQueryOptions(params?)` | `authkitKeys.admin.audit(params)` | `{ type?, page?, size?, subject? }` |
