...
on the [Observability](/docs/observability#telescope--the-agora-diagnostics-bus) page.
...
[Client](/docs/client)) produces your domain user, and Authz's `resolveUserRef(user)`
...
to be installed. See [Client → Resilience](/docs/client#resilience-for-outbound-calls).

## Durable GDPR workflows

AuthKit's [erasure cascade](/docs/compliance#account-deletion-self-service) is
...
<Callout type="info">
  `ctx.localStep` runs the body **in-process** and checkpoints its result — the right primitive
  when the erasure code lives in the same app. `ctx.step` is a different thing: it is always
  *dispatched*, and its second argument is the step's **input**, not a body — use it (with a
  `@Step` reference or a step name) when the purge runs on a separate worker. See
  [Workflows & steps](/docs/durable/concepts/workflows-and-steps).
</Callout>
...
<Callout type="info">
  Use the **embedded** [SDK driver](/docs/sdk) inside the same app, or `remote` when
