...
| `accountSession` | `{ acceptIdpSession?: boolean }` | `{ acceptIdpSession: false }` | SSO: the account console accepts the active IdP session instead of asking for a second login. See [Console session](/docs/console-session#single-sign-on-with-the-idp-session). |
...
| `node ace authkit:users:normalize-emails [--apply]` | Migrate stored addresses to the normalized identity form (`trim` + `toLowerCase`). Reports by default; `--apply` writes, refusing to touch accounts involved in a collision. **Required upgrade step** from 0.70 or earlier — see [below](#authkitusersnormalize-emails). |
...
### `authkit:users:normalize-emails`

The login looks up **one** form of the address — `trim()` + `toLowerCase()`. Accounts
stored under any other spelling (the pre-0.69 mangled form, or mixed case from an import,
an invitation or a social provider) are unreachable, silently. This command is what brings
them back, and it is a **required step when upgrading from 0.70 or earlier** — the
`login.legacyEmailFallback` bridge that used to cover them at login time is gone.

Without `--apply` it writes nothing: it reports how many addresses would change, which, and
the **collisions** — two accounts that collapse onto the same normalized address. With
`--apply` it writes, refusing to touch any account involved in a collision and listing them
for a human decision; it never merges accounts and never picks a winner. Rows whose address
normalizes to nothing (a null or blank column) are listed separately rather than counted as
"already normalized" — the report never vouches for a row it left behind. It exits non-zero
when collisions (or write failures) remain. See
[Security](/docs/security#migrating-the-stored-addresses--a-required-upgrade-step).
...
| `oidcBearerGuard(...)` / `OidcBearerGuard` | Guard for APIs called with `Authorization: Bearer <access token>` (native apps, SPAs, services). Verifies in-process or as a remote resource server; 401 on failure, never a redirect. |
| `OidcBearerGuardOptions` / `OidcBearerGuardPolicy` / `OidcBearerGuardEvents` | Its option, policy (`scopes`, `audience`, `clientIds`) and event types. |
| `inProcessAccessTokenVerifier(...)` / `remoteAccessTokenVerifier(...)` | The two access-token verification strategies behind the guard (`AccessTokenVerifier`, `VerifiedAccessToken`). |

See [Native apps](/docs/native-apps) for the bearer guard.

### Native clients (RFC 8252)

| Export | Purpose |
| --- | --- |
| `ApplicationType` | `'web' \| 'native'` — `ClientConfig.applicationType` / Admin API `applicationType`. |
| `redirectUriProblem(uri, type?)` | Why a redirect URI is invalid for a client type, or `null`. |
| `assertClientMetadata(input)` / `ClientMetadataError` | The type-aware checks `AdminClientsService` runs before persisting a client (the Admin API maps the error to `422 invalid_client_metadata`). |
