...
| `login` | `LoginConfigInput` | all off; `legacyEmailFallback` on | Login policy: `requireVerifiedEmail` (capability-probed), `otp` (six-digit email codes — see [Passwordless](/docs/passwordless#email-login-codes-otp)) and `legacyEmailFallback` (temporary bridge for accounts stored under the pre-0.69 mangled address — see [Security](/docs/security#email-identity-and-normalization)). **Declaring `login.requireVerifiedEmail` locks the `require_verified_email` setting.** |
